...
export function analytics(days: number): Analytics {
  ...
  let db: Database
  try { db = new Database(hermesPath("state.db"), { readonly: true }) }
  catch { return ZERO }
  try {
    const tot = db.query(
      `SELECT COUNT(*) n,
    ...
    // the sessions join; cheap at herm DB sizes (tens of thousands of
    // rows). If json_each is unavailable (older sqlite) the catch
    // returns []; the tab degrades to tool_call_count only.
    let tools: Row[] = []
    try {
      tools = db.query(
